...
Vue.component('linechart', {
  ...
  methods: {
    getPoints() {
      ...
      for (let i = 0; i < len; ++i) {if (window.CP.shouldStopExecution(0)) break;
        let d = this.values[i];
...
new Vue({
  ...
  methods: {
    ...
    sortBy(key, order) {
      if (this.sort !== key) {this.order = order || 'asc';} else
    ...
    fakeHistory(close) {
      ...
      for (let i = 0; i < 50; ++i) {if (window.CP.shouldStopExecution(1)) break;
